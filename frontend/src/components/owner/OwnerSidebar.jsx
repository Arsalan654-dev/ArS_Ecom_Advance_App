...
const navItems = [
  ...
  { to: "/owner/restaurant", label: "Restaurant", icon: MdStore },
...
export const OwnerSidebar = ({ collapsed = false, mobileOpen = false, onMobileClose }) => {
  ...
  const businessName = user?.businessName || "Restaurant Owner";
  const initials = (businessName || "R").charAt(0).toUpperCase();

  const baseLink = "flex items-center gap-3 px-4 py-2.5 rounded-lg font-medium transition-colors cursor-pointer";
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-white/10 shrink-0">
        <div className="flex items-center gap-3">
          ...
          {!collapsed && (
            <div>
              <div className="text-white font-bold text-lg leading-none">Vingo Owner</div>
              <div className="text-violet-200 text-xs mt-0.5 truncate max-w-[140px]">{businessName}</div>
            </div>
          )}
        </div>
        <button
